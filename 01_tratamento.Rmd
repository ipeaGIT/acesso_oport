...
# rais1 <- setDT(rais)[precisiondepth != "1 Estrela", .(precisiondepth, 
#                                                       clas_cnae10,
...
rais<-left_join(rais,geral3,by="cnae.setor")
...
setDT(rais)[, ':='(lon = str_replace(lon, ",", "."),
                   lat = str_replace(lat, ",", "."))]

setDT(rais)[, ':='(lon = as.numeric(lon),
                   lat = as.numeric(lat))]

rais_fim <- rais[qt_vinc_ativos2 > 0]

# Salvar
fwrite(rais_fim, "../data/rais/rais_2017_corrigido.csv")
write_rds(rais_fim, "../data/rais/rais_2017_corrigido.rds")

# Teste
# setDT(rais)[cnae.setor=="75"]
