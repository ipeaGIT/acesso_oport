...
-   `lon` e `lat`: informações de localização.
...
-   Selecionar as colunas de interesse: `id_estab`, que é o id do estabelecimento, `cidade_uf`, que é a cidade daquele emprego, e coordenadas.
-   Salvar para o arquivo `rais_2015.rds`.

``` r
rais_raw <- read_csv("../data-raw/rais/rais_2015_rafa_franco.csv")

rais <- rais_raw %>%
  # Excluir enderecos com 1 estrela
  filter(PrecisionDepth != "1 Estrela") %>%
  # Tirar acentos do endereco e transformar tudo para minusculo
  mutate(MatchedAddress = iconv(MatchedAddress, to = "ASCII//TRANSLIT")) %>%
  mutate(MatchedAddress = tolower(MatchedAddress)) %>%
  # Extrair cidade
  mutate(cidade_uf = gsub("(^.+,\\s)?(\\D+,\\s[[:alpha:]]{2})(,\\s\\d+$)?", "\\2", MatchedAddress, perl = TRUE)) %>%
  # Selecionar colunas
  select(id_estab, cidade_uf, lon = Longitude, lat = Latitude) %>%
  # Tirar nulos
  filter(!is.na(lon)) %>%
...
write_rds(rais, "../data/rais/rais_2015.rds")
```

Analisar a qualidade do georreferenciamento para as grandes cidades:

``` r
cidades <- c("fortaleza, ce", "recife, pe", "belo horizonte, mg", "rio de janeiro, rj",
                                         "porto alegre, rs", "curitiba, pr", "teresina, pi")

rais %>%
  filter(cidade_uf %in% cidades) %>%
  count(PrecisionDepth, cidades_uf) %>%
  ggplot()+
  geom_bar(aes(x = cidades_uf, y = n, fill = PrecisionDepth))
```
