...
aplicar_otp <- function(cidade, data, res = "08") {
...
# aplicar otp para todos os modos
aplicar_otp("for", data = dia, res = "09")
...
# aplicar otp para todos os modos
aplicar_otp("bel", data = dia, res = "09")
...
# aplicar otp para todos os modos
aplicar_otp("rio", data = dia, res = "09")
...
# aplicar otp para todos os modos
aplicar_otp("por", data = dia, res = "09")
...
Verificação desses pontos para Fortaleza:

```{r for_erro}

# abrir street network
path_network <- dir("../otp/graphs/for", full.names = TRUE, pattern = "*.pbf$")
st_layers(path_network)
network <- st_read(path_network, layer = "lines") %>%
  # Selecionar somente vias
  filter(!is.na(highway))


# extrair pontosque nao foram roteados pelo otp
points_fora <- avaliar_qualidade_otp("for") %>%
  select(id_hex, X, Y) %>%
...
# viz
mapview(points_fora %>% st_as_sf(coords = c("X", "Y"), crs = 4326), zcol = NULL) + mapview(network_filtrada)

# checar atividades nesse ponto
atividades <- read_rds("../data/hex_agregados/hex_agregado_for_09.rds") %>%
  filter(id_hex %in% points_fora$id_hex)

# Diagnostico: o unico que ficou fora está localizado no mar, mas tem população
  

# aplicar funcao?
source("R/fun/snap_point_to_road.R")

teste <- lapply(points_fora$id_hex, 
                snap_sf, 
                points_to_correct = points_fora, streets_buffer = network_buffer)
...
Verificação desses pontos para Belo Horizonte:

```{r for_erro}

# abrir street network
path_network <- dir("../otp/graphs/bel", full.names = TRUE, pattern = "*.pbf$")
st_layers(path_network)
network <- st_read(path_network, layer = "lines") %>%
  # Selecionar somente vias
  filter(!is.na(highway))
...
# extrair pontosque nao foram roteados pelo otp
points_fora <- avaliar_qualidade_otp("bel") %>%
  select(id_hex, X, Y) %>%
  distinct(id_hex, .keep_all = TRUE)
...
# checar atividades nesses pontos
atividades <- read_rds("../data/hex_agregados/hex_agregado_bel_09.rds") %>%
  filter(id_hex %in% points_fora$id_hex)
