...
# sigla_muni <- 'cam'; ano <- 2018; modo <- "todos"; from = 7; until = 8; every = 15; time_threshold = 7200; max_walk_distance = 800
...
criar_script_python_paral_modes_muni <- function(sigla_muni,
  ...
  # purrr::walk(dir(sprintf("../../otp/py/%s/%s", ano, sigla_muni), full.names = TRUE), file.remove)
  ...
  if (modo %in% c("todos", "tp")) {
    ...
    if (ano %in% c(2019) & sigla_muni %like% 'bho') {
      
      data <- '2019-09-20'
      
    } else {
      
      data <- selecionar_data_gtfs(sigla_muni, ano = ano)
      
    }
    
    ano1 <- substr(data, 1, 4)
  ...
  sigla_muni_fix <- substr(sigla_muni, 1, 3)
  
  message(paste0("Trabalhando na cidade ",sigla_muni, ' para o ano ', ano, "\n"))
  ...
  comum <- c(
    ...
    sprintf("otp = OtpsEntryPoint.fromArgs(['--graphs', 'graphs/%s', '--router', '%s'])", ano, sigla_muni),
    "",
  ...
  transit <- c(
    ...
    "gc.collect()",
    "",
    sprintf("filename = 'points/%s/points_%s_09_%s.csv'", ano, sigla_muni_fix, ano),
    "batch_size = 128",
    "files = []",
    "try:",
    "  os.mkdir('temp_data')",
    ...
    "",
    "      # Save the result",
    sprintf("      matrixCsv.save('../data/acesso_oport/output_ttmatrix/%s/%s/ttmatrix_%s_pt_%s_'+ str(h)+\"-\"+str(m) + \"_\" + p.split('/')[1])", ano, sigla_muni, sigla_muni, ano),
    "      gc.collect()",
    "",
    "",
  ...
  active <- c(
    ...
    "gc.collect()",
    "",
    sprintf("filename = 'points/%s/points_%s_09_%s.csv'", ano, sigla_muni, ano),
    "batch_size = 128",
    "files = []",
    "try:",
    "  os.mkdir('temp_data')",
    ...
    "",
    "      # Save the result",
    sprintf("      matrixCsv.save('../data/acesso_oport/output_ttmatrix/%s/%s/ttmatrix_%s_walk_%s_'+ str(h)+\"-\"+str(m) + \"_\" + p.split('/')[1])", ano, sigla_muni, sigla_muni, ano),
    "      gc.collect()",
    "",
    "",
    "# ^ that ^ function has to be defined before it's called",
    ...
    "",
    "      # Save the result",
    sprintf("      matrixCsv.save('../data/acesso_oport/output_ttmatrix/%s/%s/ttmatrix_%s_bike_%s_'+ str(h)+\"-\"+str(m) + \"_\" + p.split('/')[1])", ano, sigla_muni, sigla_muni, ano),
    "      gc.collect()",
    "",
    "",
    "# ^ that ^ function has to be defined before it's called",
  ...
  if (!dir.exists(sprintf("../../otp/py/%s/%s", ano, sigla_muni))) {
  ...
  if (modo %in% c("todos", "tp")) {
    ...
    output_file <- sprintf("../../otp/py/%s/%s/otp_%s_%s-%s_%s-%s.py", ano, sigla_muni, sigla_muni, ano, mes, from, until)
    
  } else {
    
    output_file <- sprintf("../../otp/py/%s/%s/otp_%s_%s.py", ano, sigla_muni, sigla_muni, ano)
