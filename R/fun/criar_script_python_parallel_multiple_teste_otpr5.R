...
# COMEÇA AQUI O SCRIPT ----------------------------------------------------
  ...
  "",
  "      # Save the result",
  sprintf("      matrixCsv.save('../data/acesso_oport/output_ttmatrix/%s/%s/ttmatrix_%s_pt_'+ str(h)+\"-\"+str(m) + \"_\" + p.split('/')[1])", "teste_otp_r5", sigla_muni, sigla_muni),
  "      gc.collect()",
  "",
  "",
  ...
  "",
  "      # Save the result",
  sprintf("      matrixCsv.save('../data/output_ttmatrix/%s/%s/ttmatrix_%s_walk_%s_'+ str(h)+\"-\"+str(m) + \"_\" + p.split('/')[1])", ano, sigla_muni, sigla_muni, ano),
  "      gc.collect()",
  "",
  "",
  "# ^ that ^ function has to be defined before it's called",
  ...
  "",
  "      # Save the result",
  sprintf("      matrixCsv.save('../data/output_ttmatrix/%s/%s/ttmatrix_%s_bike_%s_'+ str(h)+\"-\"+str(m) + \"_\" + p.split('/')[1])", ano, sigla_muni, sigla_muni, ano),
  "      gc.collect()",
  "",
  "",
  "# ^ that ^ function has to be defined before it's called",
  ...
  output_file <- sprintf("../../otp/py/%s/%s/otp_%s_%s-%s_%s-%s.py", "teste_otp_r5", sigla_muni, sigla_muni, ano, mes, from, until)
  
  } else {
    
  output_file <- sprintf("../../otp/py/%s/%s/otp_%s_%s.py", "teste_otp_r5", sigla_muni, sigla_muni, ano)
