...
summary_geocode <- function(ano) {
  ...
  siglas <- munis_list$munis_metro[ano_metro == ano] %>% tidyr::unnest(code_muni) %>% dplyr::select(sigla_muni = abrev_muni, code_muni)
  
  
  # empregos ----------------------------------------------------------------
  path_antes <- sprintf("../../data/acesso_oport/rais/%s/rais_%s_etapa3_geocoded.rds", ano, ano)
  path_depois <- sprintf("../../data/acesso_oport/rais/%s/rais_%s_etapa4_geocoded_gmaps_gquality_corrected_escola.rds", ano, ano)
  antes <- read_rds(path_antes)
  depois <- read_rds(path_depois)
  antes <- antes %>% left_join(siglas %>% mutate(code_muni =   substr(code_muni, 1, 6)), by = c("codemun" = "code_muni"))
  depois <- depois %>% left_join(siglas %>% mutate(code_muni = substr(code_muni, 1, 6)), by = c("code_muni" = "code_muni"))
